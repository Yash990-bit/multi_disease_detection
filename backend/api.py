...
if MONGODB_URI:
    try:
        ...
        print(f"MongoDB Connection Failed: {e}. Running without database logging.")
        client = None
...
class DiabetesInput(BaseModel):
    pregnancies: int; glucose: float; blood_pressure: float; skin_thickness: float
    insulin: float; bmi: float; pedigree_function: float; age: int
...
def health_check():
    return {
        "status": "ok", 
        "mongodb": "connected" if logs_collection else "disabled/error",
        "api": "online"
    }
...
def api_predict_diabetes(data: DiabetesInput):
    try:
        ...
        if logs_collection:
            try:
                log = {"disease_type": "Diabetes", "input_data": data.dict(), "result": result, "timestamp": datetime.utcnow()}
                logs_collection.insert_one(log)
            except: pass # Ignore DB errors during prediction
...
def api_predict_heart(data: HeartInput):
    try:
        ...
        if logs_collection:
            try:
                log = {"disease_type": "Heart", "input_data": data.dict(), "result": result, "timestamp": datetime.utcnow()}
                logs_collection.insert_one(log)
            except: pass
...
def api_predict_liver(data: LiverInput):
    try:
        ...
        if logs_collection:
            try:
                log = {"disease_type": "Liver", "input_data": data.dict(), "result": result, "timestamp": datetime.utcnow()}
                logs_collection.insert_one(log)
            except: pass
